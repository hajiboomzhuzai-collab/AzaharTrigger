...
void NWM_UDS::HandleDeauthenticationFrame(const Network::WifiPacket& packet) {
    ...
    LOG_ERROR(Service_NWM,
        "DEAUTH sender node={} connected={} host_status={} total_nodes={}",
        node.node_id,
        node.connected,
        static_cast<int>(connection_status.status),
        connection_status.total_nodes);
    ...
    if (!node.connected) {
        LOG_DEBUG(Service_NWM,
            "Received DeauthenticationFrame from a not connected MAC Address");
        return;
    }

    auto node_it = std::find_if(node_info.begin(), node_info.end(),
